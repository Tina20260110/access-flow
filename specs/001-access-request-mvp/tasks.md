...
- [X] T048 为 AppShell 增加二次确认的“重置 Demo 数据”，成功后清领域 cache、恢复默认员工并导航列表，失败保持原数据；补集成测试，完成 `src/app/AppShell.tsx`、`src/app/app-shell.test.tsx`（Plan：重置 Demo 数据；FR-023、FR-024、FR-028）
- [X] T049 编写并运行 Chromium 核心闭环 E2E：不使用开发者工具、手工修改持久化数据或页面外操作，仅通过 UI 完成重置→员工连续表单创建→列表查找→查看详情→切换到 `approverId` 员工批准→切回 `requesterId` 员工查看最终结果，并直接核对状态、风险和当前下一步，同时覆盖纯键盘主路径，完成 `e2e/access-flow.spec.ts`（US1～US4；SC-001、SC-002、SC-004、SC-005、SC-007～SC-009）
